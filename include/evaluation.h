#pragma once

#include "game.h"
#include <array>
#include <cstdint>

namespace ChessGame {
namespace Evaluation {

constexpr std::array<int16_t, numberChessPieces + 1> pieceValues{
    20000, 900, 500, 330, 320, 100, 100, // last is a second pawn. Because of ep a NONE piece could be returned.
};

// clang-format off
constexpr int16_t pieceTableMiddleGame[numberChessPieces][64] = {
    // King
    {
        -30,-40,-40,-50,-50,-40,-40,-30,
        -30,-40,-40,-50,-50,-40,-40,-30,
        -30,-40,-40,-50,-50,-40,-40,-30,
        -30,-40,-40,-50,-50,-40,-40,-30,
        -20,-30,-30,-40,-40,-30,-30,-20,
        -10,-20,-20,-20,-20,-20,-20,-10,
         20, 20,  0,  0,  0,  0, 20, 20,
         20, 30, 10,  0,  0, 10, 30, 20
    },

    // Queen
    {
        -20,-10,-10, -5, -5,-10,-10,-20,
        -10,  0,  0,  0,  0,  0,  0,-10,
        -10,  0,  5,  5,  5,  5,  0,-10,
         -5,  0,  5,  5,  5,  5,  0, -5,
          0,  0,  5,  5,  5,  5,  0, -5,
        -10,  5,  5,  5,  5,  5,  0,-10,
        -10,  0,  5,  0,  0,  0,  0,-10,
        -20,-10,-10, -5, -5,-10,-10,-20    
   },

    // Rook
    {
        0,  0,  0,  0,  0,  0,  0,  0,
        5, 10, 10, 10, 10, 10, 10,  5,
       -5,  0,  0,  0,  0,  0,  0, -5,
       -5,  0,  0,  0,  0,  0,  0, -5,
       -5,  0,  0,  0,  0,  0,  0, -5,
       -5,  0,  0,  0,  0,  0,  0, -5,
       -5,  0,  0,  0,  0,  0,  0, -5,
        0,  0,  0,  5,  5,  0,  0,  0
    },

    // Bishop
    {
       -20,-10,-10,-10,-10,-10,-10,-20,
        -10,  0,  0,  0,  0,  0,  0,-10,
        -10,  0,  5, 10, 10,  5,  0,-10,
        -10,  5,  5, 10, 10,  5,  5,-10,
        -10,  0, 10, 10, 10, 10,  0,-10,
        -10, 10, 10, 10, 10, 10, 10,-10,
        -10,  5,  0,  0,  0,  0,  5,-10,
        -20,-10,-10,-10,-10,-10,-10,-20,
   },

    // Knight
    {
        -50,-40,-30,-30,-30,-30,-40,-50,
        -40,-20,  0,  0,  0,  0,-20,-40,
        -30,  0, 10, 15, 15, 10,  0,-30,
        -30,  5, 15, 20, 20, 15,  5,-30,
        -30,  0, 15, 20, 20, 15,  0,-30,
        -30,  5, 10, 15, 15, 10,  5,-30,
        -40,-20,  0,  5,  5,  0,-20,-40,
        -50,-40,-30,-30,-30,-30,-40,-50,
    },

    // Pawn
    {
        0,  0,  0,  0,  0,  0,  0,  0,
        50, 50, 50, 50, 50, 50, 50, 50,
        10, 10, 20, 30, 30, 20, 10, 10,
         5,  5, 10, 25, 25, 10,  5,  5,
         0,  0,  0, 20, 20,  0,  0,  0,
         5, -5,-10,  0,  0,-10, -5,  5,
         5, 10, 10,-20,-20, 10, 10,  5,
         0,  0,  0,  0,  0,  0,  0,  0
},
};
// clang-format on

constexpr auto calculate_piece_table() {
    std::array<std::array<std::array<int16_t, 64>, numberChessPieces>, 2> pt{};
    for (uint8_t p = 0; p < numberChessPieces; p++) {
        for (Position pos = 0; pos < 64; pos++) {
            uint8_t rank = rank_from_pos(pos);
            uint8_t file = file_from_pos(pos);
            rank = 7 - rank;
            int16_t value = pieceTableMiddleGame[p][pos];
            pt[1][p][pos] = value;
            pt[0][p][coords_to_pos(file, rank)] = value;
        }
    }
    return pt;
}

constexpr std::array<std::array<std::array<int16_t, 64>, numberChessPieces>, 2> piece_table = calculate_piece_table();

void show_piece_square_table(const std::array<int16_t, 64> &squares);
int32_t evaluate(Game &game);

} // namespace Evaluation
} // namespace ChessGame
