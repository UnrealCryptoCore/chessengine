...
constexpr int32_t pawn_phase = 0;
constexpr int32_t knight_phase = 1;
constexpr int32_t bishop_phase = 1;
constexpr int32_t rook_phase = 2;
constexpr int32_t queen_phase = 4;
constexpr int32_t total_phase =
    pawn_phase * 16 + knight_phase * 4 + bishop_phase * 4 + rook_phase * 4 + queen_phase * 2;
constexpr std::array<int32_t, numberChessPieces - 1> piece_phases{
    queen_phase, rook_phase, bishop_phase, knight_phase, pawn_phase,
};
...
constexpr std::array<std::array<int, 64>, numberChessPieces> mg_pesto_table{
    mg_king_table, mg_queen_table, mg_rook_table, mg_bishop_table, mg_knight_table, mg_pawn_table,
};

constexpr std::array<std::array<int, 64>, numberChessPieces> eg_pesto_table{
    eg_king_table, eg_queen_table, eg_rook_table, eg_bishop_table, eg_knight_table, eg_pawn_table,
};

template <std::array<std::array<int, 64>, numberChessPieces> table>
constexpr auto calculate_piece_table() {
    std::array<std::array<std::array<int32_t, 64>, numberChessPieces>, 2> pt{};
    for (uint8_t p = 0; p < numberChessPieces; p++) {
        for (Position pos = 0; pos < 64; pos++) {
            int16_t value = table[p][pos];
            pt[1][p][pos] = value;
            pt[0][p][flip(pos)] = value;
        }
    }
    return pt;
}

constexpr std::array<std::array<std::array<int32_t, 64>, numberChessPieces>, 2> mg_piece_table =
    calculate_piece_table<mg_pesto_table>();

constexpr std::array<std::array<std::array<int32_t, 64>, numberChessPieces>, 2> eg_piece_table =
    calculate_piece_table<eg_pesto_table>();
